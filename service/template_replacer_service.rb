class TemplateReplacerService < Service
  ...
  # Replace all tokens in a template with their data
  ...
  def self.perform_template_replacements template
    template.gsub(TOKEN_REGEX) do |token|
      replacement = TokenService.replacement_for token
  ...
  def self.parse_context_and_attribute naked_token
    ...
    #todo maybe handle nested attributes, e.g. [[user.mom.phone_number]]
    naked_token.split '.'
  end
